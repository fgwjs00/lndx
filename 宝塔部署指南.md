...
> **停止使用本文档执行部署。** 本文档保留仅用于历史追溯，其中“上传整个项目”、
> “运行 `baota-deploy.sh`”、自动初始化数据库和默认测试账号等步骤均不适用于当前
> 线上系统。正式部署必须使用
> [`docs/deployment/baota-migration-runbook.md`](docs/deployment/baota-migration-runbook.md)，
> 先完成生产备份恢复、迁移演练、构建验证和复制清单检查，再按清单上传。
...
生产环境不创建或公布默认测试账号。管理员账号必须通过受控初始化流程创建，密码不得
写入仓库、部署文档或构建产物。
