...
async function assertPhase2TablesExist(client) {
  const rows = await client.$queryRawUnsafe(
    "SELECT to_regclass('public.class_sections')::text AS class_sections, to_regclass('public.semesters')::text AS semesters"
  )
  const state = rows[0] || {}
  if (!state.class_sections || !state.semesters) {
...
async function ensureClassSection(tx, academicYearId, semesterId, toSemester, course) {
  const now = new Date()
  const coursePart = safeCodePart(course.courseCode || course.id.slice(0, 8))
  const code = `${buildSemesterCode(toSemester)}-${coursePart}`
  const rows = await tx.$queryRaw`
    ...
    VALUES (
      ...
      ${JSON.stringify(course.timeSlots || [])}::jsonb, 'DRAFT', true, ${now}, ${now}
    )
    ON CONFLICT ("code") DO UPDATE SET
      "name" = EXCLUDED."name",
      "academicYearId" = EXCLUDED."academicYearId",
...
async function main() {
  ...
  if (!fromSemester || !toSemester) {
    throw new Error('Usage: node scripts/enrollment-phase2-prepare-next-sections.js --from-semester "2025年秋季" --to-semester "2026年秋季" [--execute]')
  }
  ...
  console.log('Enrollment phase 2 next semester class sections')
  ...
  const courses = await getSourceCourses(fromSemester)
  console.log(`Source published courses: ${courses.length}`)
  ...
  const result = await prisma.$transaction(async tx => {
    ...
    for (const course of courses) {
      await ensureClassSection(tx, academicYearId, semesterId, toSemester, course)
      classSectionCount += 1
    }
  ...
  console.log('\nNext semester class sections completed.')
