...
const enrollmentConfig_1 = require("../utils/enrollmentConfig");
const router = (0, express_1.Router)();
const prisma = new client_1.PrismaClient();
// 验证schema
const applicationV2Schema = joi_1.default.object({
    // 学生基本信息（使用前端字段名）
    name: joi_1.default.string().required().messages({
        'string.empty': '请输入真实姓名',
        'any.required': '真实姓名为必填项'
    }),
    gender: joi_1.default.string().valid('男', '女', 'MALE', 'FEMALE').required().messages({
        'any.only': '性别只能是男、女、MALE或FEMALE',
        'any.required': '性别为必填项'
    }),
    idNumber: joi_1.default.string().pattern(/^[1-9]\d{5}(18|19|20)\d{2}((0[1-9])|(1[0-2]))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$/).required().messages({
        'string.pattern.base': '身份证号格式不正确',
        'any.required': '身份证号为必填项'
    }),
    birthDate: joi_1.default.string().isoDate().required().messages({
        'string.isoDate': '出生日期格式不正确',
        'any.required': '出生日期为必填项'
    }),
    contactPhone: joi_1.default.string().pattern(/^1[3-9]\d{9}$/).required().messages({
        'string.pattern.base': '手机号格式不正确',
        'any.required': '联系电话为必填项'
    }),
    major: joi_1.default.string().allow('').messages({
        'string.base': '专业必须是字符串'
    }),
    idCardAddress: joi_1.default.string().allow('').messages({
        'string.base': '身份证地址必须是字符串'
    }),
    emergencyContact: joi_1.default.string().allow('').messages({
        'string.base': '紧急联系人必须是字符串'
    }),
    emergencyPhone: joi_1.default.string().allow('').messages({
        'string.base': '紧急联系人电话必须是字符串'
    }),
    // 工作信息
    isRetired: joi_1.default.boolean().required().messages({
        'any.required': '工作状态为必填项'
    }),
    insuranceCompany: joi_1.default.string().optional().allow('').messages({
        'string.base': '保险公司必须是字符串'
    }),
    retirementCategory: joi_1.default.string().optional().allow('').messages({
        'string.base': '保险类别必须是字符串'
    }),
    // 课程信息
    semester: joi_1.default.string().required().messages({
        'any.required': '学期为必填项'
    }),
    selectedCourses: joi_1.default.array().items(joi_1.default.string()).min(1).max(2).required().messages({
        'array.min': '请选择至少一门课程',
        'array.max': '最多只能选择2门课程',
        'any.required': '课程选择为必填项'
    }),
    // 学习期间（保险信息为可选）
    studyPeriodStart: joi_1.default.string().isoDate().allow('', null).optional().messages({
        'string.isoDate': '保险开始日期格式不正确'
    }),
    studyPeriodEnd: joi_1.default.string().isoDate().allow('', null).optional().messages({
        'string.isoDate': '保险结束日期格式不正确'
    }),
    // 其他信息
    remarks: joi_1.default.string().allow('').messages({
        'string.base': '备注必须是字符串'
    }),
    photo: joi_1.default.string().allow('').messages({
        'string.base': '照片必须是字符串'
    }),
    // 前端发送的其他字段（允许存在但不强制验证）
    ethnicity: joi_1.default.string().allow('').messages({
        'string.base': '民族必须是字符串'
    }),
    healthStatus: joi_1.default.string().allow('').messages({
        'string.base': '健康状况必须是字符串'
    }),
    educationLevel: joi_1.default.string().allow('').messages({
        'string.base': '学历必须是字符串'
    }),
    politicalStatus: joi_1.default.string().allow('').messages({
        'string.base': '政治面貌必须是字符串'
    }),
    phone: joi_1.default.string().allow('').messages({
        'string.base': '电话必须是字符串'
    }),
    idCardFront: joi_1.default.string().allow('').messages({
        'string.base': '身份证正面照片必须是字符串'
    }),
    idCardBack: joi_1.default.string().allow('').messages({
        'string.base': '身份证背面照片必须是字符串'
    }),
    familyAddress: joi_1.default.string().allow('').messages({
        'string.base': '家庭地址必须是字符串'
    }),
    familyPhone: joi_1.default.string().allow('').messages({
        'string.base': '家庭电话必须是字符串'
    }),
    emergencyRelation: joi_1.default.string().allow('').messages({
        'string.base': '紧急联系人关系必须是字符串'
    }),
    agreementSigned: joi_1.default.boolean().allow('').messages({
        'boolean.base': '协议签署状态必须是布尔值'
    }),
    studentId: joi_1.default.string().allow('').messages({
        'string.base': '学生ID必须是字符串'
    }),
    applicationDate: joi_1.default.string().allow('').messages({
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (existingStudent) {
                ...
                // 检查现有学生的报名冲突
                const activeEnrollments = existingStudent.enrollments.filter((e) => e.status === 'PENDING' || e.status === 'APPROVED');
                // 🔧 动态检查课程数量限制（根据学期确定）
                // 获取所选课程的学期信息来确定限制
                ...
                if (applicationData.selectedCourses.length > 0) {
                    ...
                    if (firstSelectedCourse?.semester) {
                        maxCoursesAllowed = (0, enrollmentConfig_1.getMaxCoursesForSemester)(firstSelectedCourse.semester);
                    }
                }
                // 使用动态限制进行验证
                if (activeEnrollments.length + applicationData.selectedCourses.length > maxCoursesAllowed) {
                    ...
                    throw new errorHandler_1.ValidationError(`您最多只能同时报名${maxCoursesAllowed}门课程${semesterNote}，当前已有${activeEnrollments.length}门课程，无法再报名${applicationData.selectedCourses.length}门`);
                }
                for (const courseId of applicationData.selectedCourses) {
                    ...
                    if (targetCourse.enrollments.length >= targetCourse.maxStudents) {
                        throw new errorHandler_1.ValidationError(`课程"${targetCourse.name}"名额已满（${targetCourse.enrollments.length}/${targetCourse.maxStudents}）`);
                    }
                    // 1. 检查是否已报名该课程且状态为PENDING或APPROVED
                    const hasActiveEnrollment = existingStudent.enrollments.some((enrollment) => enrollment.courseId === courseId &&
                        (enrollment.status === 'PENDING' || enrollment.status === 'APPROVED'));
                    if (hasActiveEnrollment) {
                        throw new errorHandler_1.ValidationError(`您已经报名过课程"${targetCourse.name}"，请等待审核结果`);
                    }
                    // 2. 检查是否已报名该课程且被拒绝（不能重新报名被拒绝的同一门课程）
                    const hasRejectedEnrollment = existingStudent.enrollments.some((enrollment) => enrollment.courseId === courseId &&
                        enrollment.status === 'REJECTED');
                    if (hasRejectedEnrollment) {
                        throw new errorHandler_1.ValidationError(`课程"${targetCourse.name}"已被拒绝，无法重新报名。您可以选择报名其他课程`);
                    }
                    // 3. 检查是否已报名同一门课程的其他年级
                    const hasSameCourseConflict = existingStudent.enrollments.some((enrollment) => {
                    ...
                    if (hasSameCourseConflict) {
                        throw new errorHandler_1.ValidationError(`您已经报名过"${targetCourse.name}"的其他年级，不能重复报名`);
                    }
                    // 检查学生是否有任何通过审核的课程
                    const hasApprovedCourses = existingStudent.enrollments.some((enrollment) => enrollment.status === 'APPROVED');
                    // 年级权限检查
                    const gradeCheck = (0, gradeManagement_1.canEnrollCourse)(existingStudent.currentGrade, targetCourse.level, existingStudent.graduationStatus, targetCourse.requiresGrades, hasApprovedCourses);
                    if (!gradeCheck.canEnroll) {
                        throw new errorHandler_1.ValidationError(`报名失败: ${gradeCheck.reason}`);
                    }
                }
                // 更新现有学生信息（暂时简化毕业生检查）
                const isGraduated = existingStudent.graduationStatus === 'GRADUATED' ||
                    existingStudent.graduationStatus === 'ARCHIVED';
                if (isGraduated) {
                    // 根据第一门选择的课程获取院系信息
                    ...
                    const studentData = {
                        ...
                        gender: (applicationData.gender === '男' || applicationData.gender === 'MALE') ? 'MALE' : 'FEMALE',
                    ...
                    // 在读学生，检查年级升级（暂时简化）
                    const studentEnrollmentSemester = existingStudent.enrollmentSemester || currentSemester;
                    const expectedGrade = (0, gradeManagement_1.calculateCurrentGrade)(studentEnrollmentSemester, currentSemester);
                    const shouldGrad = (0, gradeManagement_1.shouldGraduate)(studentEnrollmentSemester, currentSemester);
                    // 根据第一门选择的课程获取院系信息
                    let studentMajor = existingStudent.major || '未设置';
                    if (applicationData.selectedCourses.length > 0) {
                        const firstCourse = await tx.course.findUnique({
                    ...
                    else if (expectedGrade !== existingStudent.currentGrade) {
                ...
                console.log('➕ 创建新学生');
                // 🔧 创建新学生，传递学期参数生成编号
                const studentCode = await (0, studentCodeGenerator_1.generateStudentCode)(applicationData.semester);
                // 根据第一门选择的课程获取院系信息
            ...
            for (const courseId of applicationData.selectedCourses) {
                ...
                const enrollment = await tx.enrollment.create({
                    data: {
                        enrollmentCode: await (0, codeGenerator_1.generateApplicationCode)(),
                        studentId: student.id,
                        courseId: courseId,
        ...
        // 记录操作日志
        logger_1.businessLogger.userAction(req.user.id, 'APPLICATION_SUBMIT_V2', {
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (existingStudent) {
                ...
                if (!systemUser) {
                    throw new errorHandler_1.BusinessError('系统用户不存在，无法处理匿名注册');
                }
                const studentCode = await (0, studentCodeGenerator_1.generateStudentCode)(applicationData.semester);
                // 直接使用前端字段名（无需映射）
                const newStudentData = {
        ...
        // 记录操作日志
        logger_1.businessLogger.userAction('ANONYMOUS', 'ANONYMOUS_APPLICATION_SUBMIT_V2', {
            studentId: result.student.id,
            studentName: result.student.name,
            coursesCount: result.enrollments.length,
            semester: applicationData.semester,
