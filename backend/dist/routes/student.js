...
router.get('/', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (pageNum < 1 || pageSizeNum < 1 || pageSizeNum > 100) {
        throw new errorHandler_1.BusinessError('分页参数无效', 400, 'INVALID_PAGINATION');
    }
    // 构建查询条件
    const where = {
        isActive: true
    ...
    try {
                ...
                include: {
                    enrollments: {
                        where: hasSpecificFilters ? {
                            status: status && typeof status === 'string' ? status.toUpperCase() : 'APPROVED'
                        } : {
                            // 没有具体筛选时，显示所有状态的报名记录
        ...
        logger_1.businessLogger.userAction(req.user?.id, 'STUDENT_LIST_QUERY', {
            page: pageNum,
        ...
        throw new errorHandler_1.BusinessError('获取学生列表失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 获取学生统计信息
...
router.get('/statistics', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        // 并行查询各种统计数据
        const [totalStudents, approvedStudents, pendingStudents, rejectedStudents, newStudentsThisMonth] = await Promise.all([
        ...
        logger_1.businessLogger.userAction(req.user?.id, 'STUDENT_STATS_QUERY', {
            totalStudents,
        ...
        throw new errorHandler_1.BusinessError('获取学生统计失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 获取专业列表（基于课程分类）
 ...
 */
router.get('/majors', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        throw new errorHandler_1.BusinessError('获取专业列表失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 获取学期列表（基于课程学期）
 ...
 */
router.get('/semesters', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        // 从Course表获取所有学期选项
        ...
        throw new errorHandler_1.BusinessError('获取学期列表失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 创建新学生
 * POST /api/students
 */
router.post('/', auth_1.requireTeacher, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        if (!studentData.name || !studentData.contactPhone || !studentData.idNumber) {
            throw new errorHandler_1.ValidationError('姓名、联系电话和身份证号为必填项');
        }
        // 检查身份证号是否已存在
        ...
        if (existingStudent) {
            throw new errorHandler_1.ValidationError(`身份证号 ${studentData.idNumber} 已被使用`);
        }
        // 检查手机号是否已存在
        ...
        if (existingPhone) {
            throw new errorHandler_1.ValidationError(`手机号 ${studentData.contactPhone} 已被使用`);
        }
        // 🔧 生成学生编号，使用新的按学期编号系统
        const studentCode = await (0, studentCodeGenerator_1.generateStudentCode)(studentData.semester);
        // 计算年龄
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const newStudentData = {
                ...
                // 基本信息
                gender: (studentData.gender === '男' || studentData.gender === 'MALE') ? 'MALE' : 'FEMALE',
        ...
        // 记录操作日志
        logger_1.businessLogger.userAction(req.user.id, 'STUDENT_CREATE_WITH_COURSES', {
        ...
        throw new errorHandler_1.BusinessError('创建学生失败', 500, 'CREATE_ERROR');
    }
}));
/**
 * 🔧 导出学生数据 - 必须放在动态路由之前
 ...
 */
router.get('/export', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        for (const enrollment of enrollments) {
            ...
            const gender = genderMap[student.gender] || student.gender;
            const statusMap = {
                'PENDING': '待审核',
                'APPROVED': '已通过',
                'REJECTED': '已拒绝',
            ...
            const enrollmentStatus = statusMap[enrollment.status] || enrollment.status;
            const row = [
                student.studentCode || '',
                student.name || '',
                gender,
        ...
        logger_1.businessLogger.userAction(req.user.id, 'STUDENT_DATA_EXPORT', {
        ...
        throw new errorHandler_1.BusinessError('导出学生数据失败', 500, 'EXPORT_ERROR');
    }
}));
/**
 * 获取学生详情
...
router.get('/:id', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        if (!student) {
            throw new errorHandler_1.BusinessError('学生不存在', 404, 'STUDENT_NOT_FOUND');
        }
        // 格式化学生详情数据，包含所有报名表信息
        ...
        logger_1.businessLogger.userAction(req.user?.id, 'STUDENT_DETAIL_QUERY', {
            studentId: id,
        ...
        throw new errorHandler_1.BusinessError('获取学生详情失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 更新学生信息
 * PUT /api/students/:id
 */
router.put('/:id', auth_1.requireTeacher, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const { name, contactPhone, currentAddress, emergencyContact, emergencyPhone, 
    // 身份证件
    idCardFront, idCardBack, 
    // 保险信息
    insuranceCompany, retirementCategory, studyPeriodStart, studyPeriodEnd, 
    // 学籍信息
    semester, major } = req.body;
    try {
        // 构建更新数据，只包含有值的字段
        const updateData = {
            updatedAt: new Date()
        ...
        if (idCardFront !== undefined)
            updateData.idCardFront = idCardFront;
        if (idCardBack !== undefined)
            updateData.idCardBack = idCardBack;
        // 保险信息  
        if (insuranceCompany !== undefined)
            updateData.insuranceCompany = insuranceCompany;
        if (retirementCategory !== undefined)
            updateData.retirementCategory = retirementCategory;
        ...
        logger_1.businessLogger.userAction(req.user?.id, 'STUDENT_UPDATE', {
            studentId: id,
            studentName: name,
        ...
        throw new errorHandler_1.BusinessError('更新学生信息失败', 500, 'UPDATE_ERROR');
    }
}));
/**
 * 修改学生报名状态
 ...
 */
router.patch('/:id/status', auth_1.requireTeacher, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const { status, enrollmentId } = req.body;
    if (!['PENDING', 'APPROVED', 'REJECTED', 'CANCELLED'].includes(status)) {
        throw new errorHandler_1.BusinessError('无效的报名状态', 400, 'INVALID_STATUS');
    }
    try {
        ...
        logger_1.businessLogger.userAction(req.user?.id, 'STUDENT_ENROLLMENT_STATUS_CHANGE', {
            studentId: id,
            newStatus: status,
            enrollmentId,
            studentName: student?.name
        ...
        throw new errorHandler_1.BusinessError('修改学生报名状态失败', 500, 'UPDATE_ERROR');
    }
}));
/**
 * 删除学生档案（软删除并级联处理相关数据）
 ...
 */
router.delete('/:id', auth_1.requireTeacher, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    try {
        // 使用事务确保数据一致性
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (!existingStudent) {
                throw new errorHandler_1.BusinessError('学生不存在或已被删除', 404, 'STUDENT_NOT_FOUND');
            }
            // 记录要处理的相关数据数量
        ...
        // 记录操作日志
        logger_1.businessLogger.userAction(req.user?.id, 'STUDENT_DELETE_CASCADE', {
            studentId: id,
            studentName: result.student.name,
        ...
        throw new errorHandler_1.BusinessError('删除学生档案失败', 500, 'DELETE_ERROR');
    }
}));
/**
 * 更新学生选课
 ...
 */
router.put('/:id/courses', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const { selectedCourses } = req.body;
    if (!Array.isArray(selectedCourses)) {
        throw new errorHandler_1.ValidationError('selectedCourses 必须是数组');
    }
    ...
    try {
        ...
        // 记录操作日志
        logger_1.businessLogger.userAction(req.user.id, 'STUDENT_COURSES_UPDATE', {
            studentId: id,
            studentName: result.student.name,
        ...
        throw new errorHandler_1.BusinessError('更新学生选课失败', 500, 'COURSE_UPDATE_ERROR');
    }
}));
