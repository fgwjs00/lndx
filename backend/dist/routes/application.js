...
const studentCodeGenerator_1 = require("../utils/studentCodeGenerator");
...
router.get('/', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        const applications = enrollments.map(enrollment => ({
            ...
            avatar: enrollment.student.photo || '/uploads/id-cards/default-avatar.jpg',
        ...
        logger_1.businessLogger.userAction(req.user.id, 'APPLICATION_LIST_QUERY', {
            page: Number(page),
        ...
        throw new errorHandler_1.BusinessError('获取报名申请列表失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 审核报名申请
 ...
 */
router.get('/statistics', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        // 获取总报名数（只计算活跃学生的报名）
        ...
        logger_1.businessLogger.userAction(req.user.id, 'APPLICATION_STATS_QUERY', {
        ...
        throw new errorHandler_1.BusinessError('获取报名统计数据失败', 500, 'STATS_ERROR');
    }
}));
router.post('/:id/review', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        if (!enrollment) {
            throw new errorHandler_1.BusinessError('报名记录不存在', 404, 'ENROLLMENT_NOT_FOUND');
        }
        // 更新报名状态
        const updateData = {
        ...
        logger_1.businessLogger.userAction(req.user.id, 'APPLICATION_REVIEW', {
            enrollmentId: id,
        ...
        throw new errorHandler_1.BusinessError('审核报名申请失败', 500, 'REVIEW_ERROR');
    }
}));
/**
 * 生成日期+数字格式的申请编号
...
const storage = multer_1.default.diskStorage({
    destination: (req, file, cb) => {
        const uploadDir = path_1.default.join(process.cwd(), 'uploads', 'id-cards');
        // 确保目录存在
 ...
 * @returns 保存的文件路径
 */
function saveBase64Image(base64Data, filePrefix) {
    try {
        ...
        // 生成唯一文件名
        const fileName = `${filePrefix}${Date.now()}_${crypto_1.default.randomBytes(6).toString('hex')}.${imageType}`;
        // 确保uploads目录存在
        const uploadsDir = path_1.default.join(process.cwd(), 'uploads', 'id-cards');
 ...
 */
router.post('/upload-image', auth_1.authMiddleware, upload.single('image'), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        if (!req.file) {
            throw new errorHandler_1.ValidationError('请选择要上传的图片文件', [{ field: 'image', message: '图片文件不能为空' }]);
        }
        // 构建文件访问URL
        const fileUrl = `/uploads/id-cards/${req.file.filename}`;
        // 记录上传日志
        logger_1.businessLogger.userAction(req.user.id, 'ID_CARD_UPLOAD', {
        ...
        throw new errorHandler_1.BusinessError('图片上传失败', 500, 'UPLOAD_ERROR');
    }
}));
/**
 * 获取可报名的课程列表
 ...
 */
router.get('/available-courses', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        const availableCourses = courses
            ...
            teacher: course.teachers.map((ct) => ct.teacher.realName).join(', ') || '未指定教师',
        ...
        // 记录操作日志
        logger_1.businessLogger.userAction(req.user.id, 'AVAILABLE_COURSES_QUERY', {
        ...
        throw new errorHandler_1.BusinessError('获取可报名课程列表失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 检查身份证号是否已存在
 ...
 */
router.get('/check-id', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { idNumber } = req.query;
    if (!idNumber || typeof idNumber !== 'string') {
        throw new errorHandler_1.BusinessError('身份证号不能为空', 400, 'VALIDATION_ERROR');
    }
    try {
        ...
        throw new errorHandler_1.BusinessError('身份证号检查失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 验证报名表单数据
 ...
 */
router.post('/validate', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const data = req.body;
    const errors = [];
    // 基础字段验证
...
router.put('/:id', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        if (enrollment.status !== 'PENDING') {
            throw new errorHandler_1.BusinessError('只能编辑待审核的申请', 400, 'CANNOT_EDIT_REVIEWED');
        }
        // 更新申请信息
        const updateData = {};
        ...
        throw new errorHandler_1.BusinessError('更新申请信息失败');
    }
}));
/**
 * 匿名提交报名申请（手机端）
 ...
 */
router.post('/anonymous', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        if (!applicationData.name) {
            ...
            throw new errorHandler_1.ValidationError('姓名不能为空', [{ field: 'name', message: '姓名不能为空' }]);
        }
        console.log('✅ 姓名验证通过:', applicationData.name);
        if (!applicationData.idNumber) {
            console.log('❌ 身份证号验证失败:', applicationData.idNumber);
            throw new errorHandler_1.ValidationError('身份证号不能为空', [{ field: 'idNumber', message: '身份证号不能为空' }]);
        }
        ...
        if (!applicationData.selectedCourses || !Array.isArray(applicationData.selectedCourses) || applicationData.selectedCourses.length === 0) {
            ...
            throw new errorHandler_1.ValidationError('请至少选择一门课程', [{ field: 'selectedCourses', message: '请至少选择一门课程' }]);
        }
        ...
        if (applicationData.selectedCourses.length > 2) {
            ...
            throw new errorHandler_1.ValidationError('最多只能选择2门课程', [{ field: 'selectedCourses', message: '最多只能选择2门课程' }]);
        }
        ...
        if (existingStudent) {
            ...
            if (existingStudent.graduationStatus === 'GRADUATED' || existingStudent.graduationStatus === 'ARCHIVED') {
                ...
                for (const courseId of applicationData.selectedCourses) {
                    ...
                    // 检查是否在同一学期重复报名同一课程
                    const duplicateEnrollment = existingStudent.enrollments.find(enrollment => enrollment.courseId === courseId &&
                        enrollment.course.semester === targetCourse.semester &&
                        enrollment.status !== 'CANCELLED');
                    if (duplicateEnrollment) {
                        console.log('❌ 同学期重复报名:', targetCourse.name, targetCourse.semester);
                        throw new errorHandler_1.ValidationError(`您在${targetCourse.semester}已经报名过课程"${targetCourse.name}"`, [{
                                field: 'selectedCourses',
                                message: `您在${targetCourse.semester}已经报名过课程"${targetCourse.name}"`
                            }]);
                    }
                    // 检查学生是否有任何通过审核的课程
                    const hasApprovedCourses = existingStudent.enrollments.some((enrollment) => enrollment.status === 'APPROVED');
                    // 检查年级是否匹配
                    const gradeCheck = (0, gradeManagement_1.canEnrollCourse)(existingStudent.currentGrade, targetCourse.level, existingStudent.graduationStatus, true, hasApprovedCourses);
                    if (!gradeCheck.canEnroll) {
                        console.log('❌ 年级不匹配:', gradeCheck.reason);
                        throw new errorHandler_1.ValidationError(`课程"${targetCourse.name}": ${gradeCheck.reason}`, [{
                                field: 'selectedCourses',
        ...
        for (const courseId of applicationData.selectedCourses) {
            ...
            if (!course) {
                ...
                throw new errorHandler_1.ValidationError(`选择的课程不存在: ${courseId}`, [{ field: 'selectedCourses', message: '选择的课程中包含无效课程' }]);
            }
            if (course.status !== 'PUBLISHED') {
                console.log('❌ 课程未发布:', courseId, course.name);
                throw new errorHandler_1.ValidationError(`课程 "${course.name}" 当前不可报名`, [{ field: 'selectedCourses', message: '选择的课程中包含不可报名的课程' }]);
            }
        }
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (existingStudent) {
                ...
                if (existingStudent.graduationStatus === 'GRADUATED' || existingStudent.graduationStatus === 'ARCHIVED') {
                    ...
                    // 检查并更新学生年级（如果需要）
                    const shouldBeGrade = (0, gradeManagement_1.calculateCurrentGrade)(student.enrollmentSemester || currentSemester, currentSemester);
                    const needsGradeUpdate = shouldBeGrade !== student.currentGrade && shouldBeGrade !== 'GRADUATED';
                    if (needsGradeUpdate) {
                        student = await tx.student.update({
                            where: { id: student.id },
                            data: {
                                currentGrade: shouldBeGrade,
                                updatedAt: new Date()
                            }
                        });
                    ...
                    // 检查是否应该毕业
                    if ((0, gradeManagement_1.shouldGraduate)(student.enrollmentSemester || currentSemester, currentSemester)) {
                        student = await tx.student.update({
                            where: { id: student.id },
                            data: {
                ...
                const studentCode = await (0, studentCodeGenerator_1.generateStudentCode)(studentSemester);
                // 创建新学员
        ...
        // 记录操作日志（匿名用户）
        logger_1.businessLogger.userAction('anonymous', 'ANONYMOUS_APPLICATION_SUBMIT', {
        ...
        throw new errorHandler_1.BusinessError('报名申请提交失败，请重试');
    }
}));
/**
 * 提交报名申请（需要认证）
...
router.post('/', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        if (selectedCourses.length !== applicationData.selectedCourses.length) {
            ...
            throw new errorHandler_1.BusinessError('部分课程不存在或已停止招生', 400, 'COURSE_NOT_FOUND');
        }
        console.log('✅ 课程存在性检查通过');
        // 检查课程名额
        for (const course of selectedCourses) {
            if (course.enrollments.length >= course.maxStudents) {
                throw new errorHandler_1.BusinessError(`课程"${course.name}"名额已满`, 400, 'COURSE_FULL');
            }
        }
        // 检查时间冲突（如果选择了多门课程）
        if (selectedCourses.length > 1) {
            for (let i = 0; i < selectedCourses.length; i++) {
                for (let j = i + 1; j < selectedCourses.length; j++) {
                    ...
                    // 检查时间冲突
                    if (hasTimeSlotConflict(course1.timeSlots, course2.timeSlots)) {
                        throw new errorHandler_1.BusinessError(`课程"${course1.name}"与"${course2.name}"时间冲突`, 400, 'TIME_CONFLICT');
                    }
                }
            }
        }
        ...
        // 记录操作日志
        logger_1.businessLogger.userAction(req.user.id, 'APPLICATION_SUBMIT', {
            studentId: result.student.id,
            courseIds: applicationData.selectedCourses,
            enrollmentIds: result.enrollments.map(e => e.id)
        });
        ...
        if (error instanceof errorHandler_1.BusinessError || error instanceof errorHandler_1.ValidationError) {
            throw error;
        }
        ...
        if (error.code === 'P2002') {
            ...
            if (target && target.includes('studentCode')) {
                throw new errorHandler_1.ValidationError('学员编号重复，请重试', [{ field: 'studentCode', message: '系统繁忙，请稍后重试' }]);
            }
            else if (target && target.includes('idNumber')) {
                throw new errorHandler_1.ValidationError('该身份证号已经注册过', [{ field: 'idNumber', message: '该身份证号已经注册过' }]);
            }
            else if (target && target.includes('userId')) {
                throw new errorHandler_1.ValidationError('用户账号冲突', [{ field: 'userId', message: '账号关联失败，请重试' }]);
            }
            else {
                throw new errorHandler_1.ValidationError('数据重复冲突', [{ field: 'general', message: '提交的信息与现有记录冲突，请检查后重试' }]);
            }
        }
        throw new errorHandler_1.BusinessError('提交报名申请失败', 500, 'CREATE_ERROR');
    }
}));
/**
 * 检查两个时间段数组是否有冲突
 */
const hasTimeSlotConflict = (timeSlots1, timeSlots2) => {
...
router.get('/check-id/:idNumber', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        throw new errorHandler_1.BusinessError('检查身份证号失败', 500, 'CHECK_ERROR');
    }
}));
