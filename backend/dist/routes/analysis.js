 ...
 */
router.get('/overview', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        // 获取基础统计数据
        const [totalStudents, totalCourses, totalEnrollments, approvedEnrollments, pendingEnrollments, rejectedEnrollments, activeTeachers] = await Promise.all([
        ...
        throw new errorHandler_1.BusinessError('获取统计概览失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 获取热门课程排行
 ...
 */
router.get('/popular-courses', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        throw new errorHandler_1.BusinessError('获取热门课程排行失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 获取月度统计数据
 ...
 */
router.get('/monthly-stats', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        const now = new Date();
        const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
        const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0);
        // 获取本月数据
        const [newStudents, newEnrollments, graduatedStudents, totalEnrollments, rejectedEnrollments] = await Promise.all([
            prisma.student.count({
                where: {
                    isActive: true,
        ...
        throw new errorHandler_1.BusinessError('获取月度统计失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 获取课程分类统计
 ...
 */
router.get('/category-stats', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        // 按分类统计课程和报名情况
        ...
        // 统计每个分类的数据
        const categoryStats = categories.reduce((acc, course) => {
        ...
        throw new errorHandler_1.BusinessError('获取分类统计失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 获取系统状态信息
 ...
 */
router.get('/system-status', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        // 获取数据库连接状态
        let dbStatus = 'normal';
        let onlineUsers = 0;
        ...
        throw new errorHandler_1.BusinessError('获取系统状态失败', 500, 'QUERY_ERROR');
    }
}));
/**
 * 获取最近活动
 ...
 */
router.get('/recent-activities', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        logger_1.businessLogger.userAction(req.user?.id, 'RECENT_ACTIVITIES_QUERY', {
            limit,
        ...
        throw new errorHandler_1.BusinessError('最近活动查询失败', 500, 'RECENT_ACTIVITIES_ERROR');
    }
}));
/**
 * 获取课程分类统计
 ...
 */
router.get('/course-categories-stats', auth_1.authMiddleware, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    try {
        // 获取课程分类统计
        ...
        // 合并数据
        const stats = await Promise.all(categoryStats.map(async (stat) => {
        ...
        logger_1.businessLogger.userAction(req.user?.id, 'COURSE_CATEGORIES_STATS_QUERY', {
        ...
        throw new errorHandler_1.BusinessError('课程分类统计查询失败', 500, 'COURSE_CATEGORIES_STATS_ERROR');
    }
}));
