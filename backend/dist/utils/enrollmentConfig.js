...
exports.getMaxCoursesForSemester = getMaxCoursesForSemester;
exports.getEnrollmentLimits = getEnrollmentLimits;
exports.validateCourseSelection = validateCourseSelection;
/**
 * 获取指定学期的课程报名数量限制
 * @param semester 学期字符串，如：'2024秋季'、'2025春季'等
 * @returns 该学期允许的最大课程报名数量
 */
function getMaxCoursesForSemester(semester) {
    if (!semester) {
        return 2; // 默认限制为2门
    }
    // 2024年秋季特殊配置：允许3门课程
 ...
 * @returns 限制信息对象
 */
function getEnrollmentLimits(semester, currentEnrollmentCount = 0) {
 ...
 * @returns 验证结果
 */
function validateCourseSelection(semester, currentEnrollmentCount, newCourseCount) {
    const maxCoursesAllowed = getMaxCoursesForSemester(semester);
    ...
    if (totalAfterSelection > maxCoursesAllowed) {
        return {
            ...
            message: `您最多只能同时报名${maxCoursesAllowed}门课程，当前已有${currentEnrollmentCount}门课程，无法再报名${newCourseCount}门`
        };
    }
    return {
        isValid: true,
...
//# sourceMappingURL=enrollmentConfig.js.map