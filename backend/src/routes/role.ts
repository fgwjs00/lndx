...
router.get('/', authMiddleware, asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    throw new BusinessError('获取角色列表失败', 500, 'QUERY_ERROR')
  }
}))
...
router.get('/permissions', authMiddleware, asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    throw new BusinessError('获取权限列表失败', 500, 'QUERY_ERROR')
  }
}))
...
router.get('/:id', authMiddleware, asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (!role) {
      throw new BusinessError('角色不存在', 404, 'NOT_FOUND')
    }
    ...
    throw new BusinessError('获取角色详情失败', 500, 'QUERY_ERROR')
  }
}))
 ...
 */
router.put('/users/:userId/role', requireAdmin, asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!Object.values(UserRole).includes(role)) {
      throw new BusinessError('无效的角色', 400, 'INVALID_ROLE')
    }
    ...
    // 记录用户操作
    businessLogger.userAction((req as any).user.id, 'USER_ROLE_UPDATE', { 
      targetUserId: userId,
      oldRole: user.role,
      newRole: role,
    ...
    if (error instanceof BusinessError) throw error
    throw new BusinessError('更新用户角色失败', 500, 'UPDATE_ERROR')
  }
}))
...
router.post('/', requireAdmin, asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    // 验证必填字段
    if (!name || !key) {
      throw new BusinessError('角色名称和标识不能为空', 400, 'INVALID_INPUT')
    }

    // 检查角色标识是否已存在
    const existingRole = Object.values(ROLE_CONFIGS).find(r => r.key === key)
    if (existingRole) {
      throw new BusinessError('角色标识已存在', 400, 'ROLE_KEY_EXISTS')
    }

    // 验证权限是否有效
    if (!permissions || !Array.isArray(permissions)) {
      throw new BusinessError('请至少选择一项权限', 400, 'INVALID_PERMISSIONS')
    }

    const invalidPerms = permissions.filter(perm => !ALL_PERMISSIONS.includes(perm))
    if (invalidPerms.length > 0) {
      throw new BusinessError(`无效的权限: ${invalidPerms.join(', ')}`, 400, 'INVALID_PERMISSIONS')
    }

    // 记录用户操作
    businessLogger.userAction((req as any).user.id, 'ROLE_CREATE_ATTEMPT', { 
      roleName: name,
      roleKey: key,
    ...
    if (error instanceof BusinessError) throw error
    throw new BusinessError('创建角色失败', 500, 'CREATE_ERROR')
  }
}))
...
router.delete('/:id', requireAdmin, asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (role.isSystem) {
      // 记录用户操作
      businessLogger.userAction((req as any).user.id, 'ROLE_DELETE_ATTEMPT', { 
        roleId: id,
        roleName: role.name,
        result: 'system_role_protected'
      })

      throw new BusinessError('系统角色不能删除', 400, 'SYSTEM_ROLE_PROTECTED')
    }
    ...
    if (usersWithRole > 0) {
      throw new BusinessError(`无法删除角色，还有 ${usersWithRole} 个用户正在使用此角色`, 400, 'ROLE_IN_USE')
    }
    ...
    if (error instanceof BusinessError) throw error
    throw new BusinessError('删除角色失败', 500, 'DELETE_ERROR')
  }
}))
...
router.put('/:id', requireAdmin, asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (role.isSystem) {
      // 记录系统角色修改操作
      businessLogger.userAction((req as any).user.id, 'SYSTEM_ROLE_UPDATE', { 
        roleId: id,
        roleName: role.name,
      ...
      // 动态更新ROLE_CONFIGS（仅在运行时有效）
      ;(ROLE_CONFIGS as any)[role.key] = updatedRole
    ...
    if (error instanceof BusinessError) throw error
    throw new BusinessError('更新角色失败', 500, 'UPDATE_ERROR')
  }
}))
