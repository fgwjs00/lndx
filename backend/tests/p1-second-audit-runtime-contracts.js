...
const authApi = readWorkspace('frontend/src/api/auth.ts')
const authStore = readWorkspace('frontend/src/store/auth.ts')
...
assertIncludes(requestApi, 'skipAuthRedirect?: boolean', 'silent auth checks must be able to suppress login redirects')
assertIncludes(requestApi, 'if (error.config?.skipAuthRedirect)', 'silent auth failures must bypass the global login redirect')
assertIncludes(authApi, "request.get<UserInfo>('/auth/me', undefined, { skipAuthRedirect })", 'current-user checks must forward the silent redirect flag')
assertIncludes(authApi, "request.get<{ assetToken: string; expiresInSeconds: number }>('/assets/token', undefined, { skipAuthRedirect })", 'asset-token checks must forward the silent redirect flag')
assertIncludes(authStore, 'await refreshAssetToken(true)', 'auth initialization must refresh protected assets without hijacking public routes')
assertIncludes(authStore, 'AuthService.getCurrentUser(!forceLogoutOnError)', 'auth initialization must verify stale tokens silently')
assertIncludes(authStore, "status === 401", 'silent stale-token handling must distinguish confirmed unauthorized responses')
